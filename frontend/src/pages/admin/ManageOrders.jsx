    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {filtered.length === 0 ? (
          ...
          return (
            <div key={order.id} style={{ background: '#fff', border: '1.5px solid #e2e8f0', borderRadius: 16, padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 14 }}>
                <div>
                  ...
                  <div style={{ fontSize: 13, color: '#64748b' }}>{order.user_name} · {order.user_email}</div>
                  ...
                  {STATUS_OPTIONS.map(st => (
                    <button
                      ...
                      style={{
                        ...
                        border: '1.5px solid',
                        borderColor: order.status === st ? (STATUS_STYLE[st]?.border || '#e2e8f0') : '#e2e8f0',
                        background: order.status === st ? (STATUS_STYLE[st]?.bg || '#f1f5f9') : '#fff',
                        color: order.status === st ? (STATUS_STYLE[st]?.color || '#64748b') : '#64748b',
                        opacity: updating === order.id && order.status !== st ? 0.5 : 1,
                      }}
                    >{st}</button>
                  ))}
                </div>
